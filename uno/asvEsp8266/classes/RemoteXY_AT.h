...
#define AT_DELAY_1_SEG 1000
#define AT_DELAY_2_SEG 2000
#define AT_DELAY_5_SEG 5000
...
protected:
    ...
    while (millis() - timeOut <= delay)
    {
      if (serial->available() > 0)
      {
        ...
        if (b == 13)
        {
          ...
          bufferATPos = 0;
          k = 0;
          if (strcmp(bufferAT, answer) == 0)
          {
            return 1;
          }
          if (strcmp(bufferAT, AT_ANSWER_ERROR) == 0)
          {
            return 0;
          }
          ...
          if (strcmp(bufferAT, AT_MESSAGE_AT) == 0)
          {
            haveEcho = 1;
          }
        }
  ...
  void readATMessage()
  {
    ...
    while (serial->available() > 0)
    {
      ...
      {
        if (bufferATPos < AT_BUFFER_STR_LENGTH)
        {
          bufferAT[bufferATPos++] = b;
        }
        if (b == ':')
        {
          bufferAT[bufferATPos] = 0;
  ...
  {
    if (strcmpAT(bufferAT, AT_MESSAGE_CONNECT) == 0)
    {
      connectAT();
      return AT_MESSAGE_CONNECT;
    }
    if (strcmpAT(bufferAT, AT_MESSAGE_CLOSED) == 0)
    {
      closedAT();
      return AT_MESSAGE_CLOSED;
    }
    if (strcmpAT(bufferAT, AT_MESSAGE_CONNECT_FAIL) == 0)
    {
      closedAT();
      return AT_MESSAGE_CONNECT_FAIL;
    }
    if (strcmpAT(bufferAT, AT_MESSAGE_READY) == 0)
    {
      readyAT();
